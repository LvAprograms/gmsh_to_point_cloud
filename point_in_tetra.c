...
#define PLOCATION_EPS 1.0e-8 // Tolerance value for point location error
...
/* Solve a linear system of Ax = b where A is a 3x3 matrix */
{
  double B[3][3];
...
void PointLocation_BruteForce(
  /* Check entire domain during point location instead of using partitions */
  Mesh dm,
  long int npoints, const double xp[], long int econtaining[], double xip[], long int *found)
{
  ...
  long int points_located = 0; // counter
...
/* Returns `true` if a point is located within the domain */
{
...
/* Returns `true` if a point is located in a tetrahedron */
{
  const int nsd = 3;
  ...
  /* check whether xi, eta, zeta are valid */
  basis[0] = 1.0 - 0.5 * (1.0 + xi[0]) - 0.5 * (1.0 + xi[1]) - 0.5 * (1.0 + xi[2]);
  basis[1] = 0.5 * (1 + xi[0]);
  basis[2] = 0.5 * (1 + xi[1]);
  basis[3] = 0.5 * (1 + xi[2]);
...
void PointLocation_PartitionedBoundingBox(
  /* Locate a point within a partitioned bounding box */
  Mesh dm,
  long int npoints,
  const double xp[], long int econtaining[], double xip[],
