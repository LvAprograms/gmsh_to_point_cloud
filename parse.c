...
/* Create and initialise a CellPartition structure */
void CellPartitionCreate(CellPartition *_c)
{
  CellPartition c = NULL;

  c = (struct _p_CellPartition*)malloc(sizeof(struct _p_CellPartition)); // reserve memory for _p_CellPartition
  memset(c,0,sizeof(struct _p_CellPartition));  // initialise memory with zeros. 
  c->ncell = 0; 
  c->cell_list = NULL;
  *_c = c;
}

/* Remove CellParitition object and clean up memory */
...
/* Create and initialise a Mesh object */
...
/* Remove Mesh object and free up memory */
void MeshDestroy(Mesh *_m)
{
  ...
  free(m->vert);
  free(m->cell);
  /* Iterate over each CellPartition and destroy it */
...
/* Print out the mesh variables of Mesh `m`, types and structure (including partitions) */
{
  printf("[Mesh]\n");
  printf("         nvert: %d\n", m->nvert);
  printf("      coor_dim: %d\n", m->coor_dim);
...
/* */
{
  ...
  Mesh   mesh = NULL;
  size_t bytes_read=0; // counter for reading

  *m = NULL;
  fp = fopen(filename, "rb");
  if (!fp) { printf("parse_mesh(): File %s was not found or read\n",filename); return; }

  bytes_read = fread(&nvert, sizeof(int), 1, fp);    // read number of vertices
  bytes_read = fread(&coor_dim, sizeof(int), 1, fp); // read dimension of coordinates 
  ...
  switch (pp_cell) {
    ...
    /* Clean up memory before exiting function */
    free(coor);
    free(cell);
    fclose(fp);
    return;
  ...
  nparts = 0; // number of partitions buffer
